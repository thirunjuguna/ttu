...
<div class="container">
    ...
    <div class="row">
      <div class="col-md-12 col-md-offset-0">
            <h2 style='color:#255625;'>{{\ttu\Minidepartment::all()->find(auth()->user()->minidp)->department}} Department</h2>
            <h4 style='color:#255625;'>logged in as <span class="label label-primary">{{auth()->user()->first_name}}</span></h4>
            <div class="panel panel-warning">
                <div class="panel-heading">{{\ttu\Minidepartment::all()->find(auth()->user()->minidp)->department}} STUDENTS</div>
                ...
                <form >
                         ...
                         <table class="table">
                             ...
                             $mystudents=[];
                              foreach($students as $student){
                                  ...
                                  if(!in_array(intval($student->id),$mystudents)){
                                  array_push($mystudents,$student->id);
                                  }
                              }
                              
                                  
                             $students = \ttu\User::orderBy('id', 'desc')
                   ->whereIn('id',$mystudents)
                     ->Paginate(10);
                  ...
                  //  ->join('courses', 'courses.id', 'users.course')
                             ...
                             @foreach($students as $student)
                             <?php $dp= ttu\Department::all()->find($student->department);
                             $mini= ttu\Minidepartment::all()->where('dp_id',$student->minidp)->first();
                             $course= ttu\Course::all()->find($student->course);
                             ?>
                             <tr><br></tr>
                               <tr>
